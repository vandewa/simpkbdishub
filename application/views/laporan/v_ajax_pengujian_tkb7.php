	...
	<table class="sample" border="1px solid" style="font-size:11px;">
		...
		<?php for ($x=0;$x<6; $x++) {
				...
				<tr align="center">
					...
					for($j=0;$j<1; $j++) {
						if($i==$j){ ?>
							...
							for ($m=0;$m<2; $m++) {
								$jml = $this->laporan->getJmlTahunKendTkb7($tglawal,$tglakhir,$umur1[$i],$umur2[$j],$jns[$x],$sft[$m]);
								echo "<td align='center'>".$jml."</td>";
							}
							
							$jml = $this->laporan->getJmlTotalTahunKendTkb7($tglawal,$tglakhir,$umur1[$i],$umur2[$j],$jns[$x]);
							echo "<td align='center'>".$jml."</td>";
						}
					}
			...
			<tr align="center">
				...
				for ($m=0;$m<2; $m++) {
					$jml = $this->laporan->getJmlSifatKendTkb7($tglawal,$tglakhir,$jns[$x],$sft[$m]);
					echo "<td align='center'>".$jml."</td>";
				}
				
				$jml = $this->laporan->getJmlTotalSifatKendTkb7($tglawal,$tglakhir,$jns[$x]);
				echo "<td align='center'>".$jml."</td>";
				?>
			</tr>
		<?php } ?>
			<tr align="center">
				...
				for ($a=0;$a<5; $a++) {
					for ($b=0;$b<5; $b++) {
						for ($c=0;$c<2; $c++) {
							if($a==$b){ 
								$jml = $this->laporan->getTotalKendTkb7($tglawal,$tglakhir,$jbb1[$a],$jbb2[$b],$sft[$c]);
								echo "<td align='center'>".$jml."</td>";
							}
						}
					}
				...
				for ($m=0;$m<2; $m++) {
					$jml = $this->laporan->getTotalJmlKendTkb7($tglawal,$tglakhir,$sft[$m]);
					echo "<td align='center'>".$jml."</td>";
				}
				
				$jml = $this->laporan->getSemuaKendTkb7($tglawal,$tglakhir);
				echo "<td align='center'>".$jml."</td>";
				?>
			</tr>
		</tbody>
