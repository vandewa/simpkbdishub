...
class PDF extends FPDF
{
	...
	function Content($detail_pendaftaran){
		foreach($detail_pendaftaran as $row){
			...
			*/
			...
			$this->Image(base_url().'files/pendaftaran/'.$daftar_uji.'/'.$row->qr_kodeuji,95,280,25,0,'PNG');
		}
	}
	...
	function kodeuji($detail_pendaftaran){
		foreach($detail_pendaftaran as $row){
			...
			$this->Ln(10);
			$this->cell(120,0);
			$this->cell(44,5,'SIMPAN UNTUK PEMOHON','LTRB',0,'L');
		}
	}
	...
	function Pengujian($detail_uji){
		foreach($detail_uji as $row){
			...
			$this->setFillColor(255,255,255);
			$this->Cell(30,0);			
			$this->cell(90,0,$row->nomor_uji,0,0,'L');
			$this->Ln(15);
			$this->Cell(30,0);
