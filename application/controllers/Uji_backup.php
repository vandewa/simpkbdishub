...
class Uji extends CI_Controller {
	...
	public function hapuscatatan(){
		...
		redirect('uji/proses_pengesahan/'.$kode_uji);
	}
	...
	public function tambah_proses_pengesahan(){
		...
		if($hasil=="LULUS"){
			$aktif = 2;
			$cekcatatan = $this->pengujian->getCekCatatan($kode_uji);
			if($cekcatatan>0){
				$ctt=array(
					'aktif'=>0,
				);
				$this->model_app->updateData('tbl_uji_catatan',$ctt,$id);
			}
		...
		for($i = 0; $i < $length; $i++){
			if(!empty($_POST['perbaikan'][$i])){
				$catatan[$i]=array(
					'kode_uji'=>$kode_uji,
					'jns'=>1,
					'catatan'=>$this->input->post('perbaikan')[$i],
				);
				$this->model_app->insertData('tbl_uji_catatan',$catatan[$i]);
			}
		}
	...
	public function cetak_kartu_uji(){
		...
		$this->model_app->updateData('tbl_pendaftaran',$daftar,$idx);
		$data=array(
	...
	private function kirimnotifselesai($nama,$telp,$nokend,$nouji,$tgl,$id){
		...
		$curl = curl_init();
		$token = $api;
		$message = 'Hi '.$nama.', \n Terima kasih telah melakukan uji kendaraan di UPUBKB Dishub Kabupaten Tegal. Kendaraan anda dengan nomor uji '.$nouji.'('.$nokend.') sudah LULUS uji berkala dengan masa berlaku uji sampai dengan tanggal '.strftime("%d %B %Y", strtotime($tgl)).'. \n2 Guna meningkatkan kualitas pelayanan kami. Dimohon kesediannya untuk mengisi formulir survey kepuasan masyarakat pada link berikut : \n https://ekir.dishubapp.xyz/beranda/formikm/'.$id.' \n2 Terima kasih. \n Uji Kendaraan (E-kir) Dishub Kab. Tegal';
		$pesan = 'Hi '.$nama.', Terima kasih telah melakukan uji kendaraan di UPUBKB Dishub Kabupaten Tegal. Kendaraan anda dengan nomor uji '.$nouji.'('.$nokend.') sudah LULUS uji berkala dengan masa berlaku uji sampai dengan tanggal '.strftime("%d %B %Y", strtotime($tgl)).'. Guna meningkatkan kualitas pelayanan kami. Dimohon kesediannya untuk mengisi formulir survey kepuasan masyarakat pada link berikut : https://ekir.dishubapp.xyz/beranda/formikm/'.$id.' Terima kasih. Uji Kendaraan (E-kir) Dishub Kab. Tegal';
		$data = [
			'phone' => $telp,
			'message' => $message,
		];
