...
class Model_pengujian extends CI_Model{
	...
	function getJmlDataRFID(){
		return $this->db->query("SELECT idx FROM datarfid")->num_rows();
	}
	
	function getDaftarDataRFID($sampai,$dari){
		return $this->db->query("SELECT idx, nouji, noregistrasikendaraan, tgluji, nama, datetimepersovisual, datetimecetaksertifikat, datetimeupload,
		nokendalikartu, rfid from datarfid order by idx desc LIMIT $dari, $sampai")->result();
	}
	...
	function getDaftarPengujian($sampai,$dari){
		return $this->db->query("SELECT *, a.aktif as aktif, d.idx as penguji, d.nama as nama_penguji, c.status as status_kendaraan FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_kendaraan c
	...
	*/
	...
	function getCariBelumUji($match){
		...
		ON a.no_uji=e.no_uji inner join tbl_uji_detail f
		ON a.kode_uji=f.kode_uji where a.aktif='0' AND a.status='5' AND d.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') AND a.no_uji like '%$match%' or e.no_kendaraan like '%$match%'")->result();
	}
	...
	function getRiwayatFotoUji($no_uji){
		return $this->db->query("SELECT * FROM tbl_uji_foto where no_uji='$no_uji' order by id_uji_foto desc limit 4")->result();
	}
	...
	function getSkrgKendaraanBelumUji(){
		...
		ON a.no_uji=d.no_uji inner join tbl_kendaraan e
		ON a.no_uji=e.no_uji inner join tbl_uji_detail f
		ON a.kode_uji=f.kode_uji where a.aktif='0' AND a.status>='3' AND d.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') AND b.tgl_daftar_uji=CURDATE() order by id_uji asc")->result();
	}
	...
	function getKendaraanBelumUji(){
		...
		ON a.no_uji=d.no_uji inner join tbl_kendaraan e
		ON a.no_uji=e.no_uji inner join tbl_uji_detail f
		ON a.kode_uji=f.kode_uji where a.aktif='0' AND a.status>='3' AND d.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') order by id_uji asc")->result();
	}
	...
	function getCariTanggalBelumUji($match){
		...
		ON a.no_uji=d.no_uji inner join tbl_kendaraan e
		ON a.no_uji=e.no_uji inner join tbl_uji_detail f
		ON a.kode_uji=f.kode_uji where a.aktif='0' AND a.status>='3' AND d.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') AND b.tgl_daftar_uji='$match' order by id_uji asc")->result();
	}
	...
	function getUjiItem(){
		return $this->db->query("SELECT *, a.status as status_uji FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_pengguna c
		ON a.no_uji=c.no_uji inner join tbl_kendaraan d
		ON a.no_uji=d.no_uji inner join tbl_uji_detail e
		ON a.kode_uji=e.kode_uji inner join tbl_retribusi f
		ON a.kode_uji=f.kode_uji where status_bayar in ('0','1') AND a.aktif='0' AND c.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') and b.tgl_daftar_uji=CURDATE() order by a.kode_uji asc")->result();
	}
	
	function getSemuaUjiItem(){
		return $this->db->query("SELECT *, a.status as status_uji FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_pengguna c
		ON a.no_uji=c.no_uji inner join tbl_kendaraan d
		ON a.no_uji=d.no_uji inner join tbl_uji_detail e
		ON a.kode_uji=e.kode_uji inner join tbl_retribusi f
		ON a.kode_uji=f.kode_uji where status_bayar in ('0','1') AND a.aktif='0' AND c.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') order by id_uji desc")->result();
	}
	
	function getCariUjiItem($match){
		return $this->db->query("SELECT *, a.status as status_uji FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_pengguna c
		ON a.no_uji=c.no_uji inner join tbl_kendaraan d
		ON a.no_uji=d.no_uji inner join tbl_uji_detail e
		ON a.kode_uji=e.kode_uji inner join tbl_retribusi f
		ON a.kode_uji=f.kode_uji where status_bayar in ('0','1') AND a.aktif='0' AND c.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') and a.no_uji like '%$match%' or d.no_kendaraan like '%$match%' order by a.kode_uji asc")->result();
	}
	
	function getTglUjiItem($match){
		return $this->db->query("SELECT *, a.status as status_uji FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_pengguna c
		ON a.no_uji=c.no_uji inner join tbl_kendaraan d
		ON a.no_uji=d.no_uji inner join tbl_uji_detail e
		ON a.kode_uji=e.kode_uji inner join tbl_retribusi f
		ON a.kode_uji=f.kode_uji where status_bayar in ('0','1') AND a.aktif='0' AND c.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') and b.tgl_daftar_uji='$match' order by a.kode_uji asc")->result();
	}
	...
	// PENGESAHAN
	function getPengesahanAdmin(){
		return $this->db->query("SELECT *, c.nama as nama, d.nama as penguji FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_pengguna c
		ON a.no_uji=c.no_uji inner join penguji d
		ON a.id_penguji=d.idx where a.aktif='1' AND c.aktif='1' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') order by a.kode_uji desc")->result();
	}
	
	function getPengesahanUji($id){
		return $this->db->query("SELECT *, c.nama as nama, c.nama as penguji FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_pengguna c
		ON a.no_uji=c.no_uji where a.aktif='1' AND c.aktif='1' AND a.id_penguji='$id' AND b.jenis_uji NOT IN ('Mutasi Keluar','Numpang Keluar') order by a.kode_uji desc")->result();
	}
	
	function getProsesPengesahanUji($id){
		return $this->db->query("SELECT *,e.nama as penguji FROM tbl_uji a inner join tbl_uji_detail b
		ON a.kode_uji=b.kode_uji inner join tbl_kendaraan c
		ON a.no_uji=c.no_uji inner join tbl_pengguna d
		ON a.no_uji=d.no_uji inner join penguji e
		ON a.id_penguji=e.idx where a.aktif='1' AND d.aktif='1' AND a.kode_uji='$id'")->result();
	}
	
	function getKendaraanPerbaikan(){
		return $this->db->query("SELECT *, a.aktif as aktif, e.idx as penguji, e.nama as nama_penguji, c.nama as pemilik FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_pengguna c
		ON a.no_uji=c.no_uji inner join tbl_kendaraan d
		ON a.no_uji=d.no_uji inner join penguji e
	...
	function getCetakStiker($id){
		return $this->db->query("SELECT *, d.nama as penguji FROM tbl_uji a inner join tbl_pendaftaran b
		ON a.kode_uji=b.kode_uji inner join tbl_kendaraan c
		ON a.no_uji=c.no_uji inner join penguji d
		ON a.id_penguji=d.idx where a.kode_uji='$id'")->result();
	}
	
	function getCetakBukuUji($id){
		return $this->db->query("SELECT *, a.kode_uji as kode_uji, a.no_uji as no_uji, f.nama as penguji FROM tbl_uji a inner join tbl_uji_detail b
		ON a.kode_uji=b.kode_uji inner join tbl_pendaftaran c
		ON a.kode_uji=c.kode_uji inner join tbl_kendaraan d
		ON a.no_uji=d.no_uji inner join tbl_pengguna e
		ON a.no_uji=e.no_uji inner join penguji f
		ON a.id_penguji=f.idx where a.kode_uji='$id' and e.aktif='1'")->result();
	}
	...
	function getCatKerusakanAktif($id){
		return $this->db->query("Select * from tbl_uji_catatan where kode_uji='$id' AND jns='0' AND aktif='1'")->result();
	}
	...
	function getAktifCatatan($id){
		return $this->db->query("SELECT * FROM tbl_uji_catatan WHERE kode_uji='$id' and aktif='1'")->num_rows();
	}
	
	function getCekCatatan($cat,$id){
		return $this->db->query("SELECT * FROM tbl_uji_catatan WHERE kode_uji='$id' and catatan='$cat'")->num_rows();
	}
	...
	function getProsesBerkas($id){
		return $this->db->query("SELECT * FROM tbl_uji a inner join tbl_pengguna b
		ON a.no_uji=b.no_uji where b.aktif='1' AND a.kode_uji='$id'")->result();
	}
